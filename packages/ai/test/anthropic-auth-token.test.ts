...
describe("Anthropic-compatible user agents", () => {
	it("preserves an explicit user agent for Kimi Coding", async () => {
		...
		expect(userAgentHeaders).toEqual([["user-agent", "custom-client"]]);
	});

	it("applies the pi runtime user agent to Anthropic by default", async () => {
		await streamAnthropic(anthropicModel, context, { apiKey: "anthropic-key" }).result();

		const headers = mockState.constructorOpts?.defaultHeaders as Record<string, string>;
		expect(headers["User-Agent"]).toBe(`pi (${platform()} ${release()}; ${arch()})`);
	});
});
